...
std::vector<glm::vec3> verts((6 + 6) * MAP_W * MAP_H);
...
void genMesh() {
	...
	for (uint i = 0; i < (MAP_H * MAP_W) * step; i+= step) {
		uint frac = i / step;
...
void genWavesMap() {
	...
	// DEBUG SAVE
    lodepng_encode_file("wave.png", picture.data(), 
...
void genSimplexMap() {
    ...
    // DEBUG SAVE
    lodepng_encode_file("noise.png", picture.data(), 
    		NOISE_W, NOISE_H, LCT_GREY, 8);
}
...
int main() {
	...
	auto shaderDestroyer = [&](ShaderProgram* sp) {
		std::array<GLuint, 2> buffers({vao, vbo});
